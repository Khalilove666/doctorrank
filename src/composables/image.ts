...
export function useImage() {
    ...
    const userAvatar = computed(() =>
        userStore.user.img ? getFullImgUrl("/user/avatar/" + userStore.user.img) : avatarPlaceHolder,
    );
    const userThumbnail = computed(() =>
        userStore.user.img ? getFullImgUrl("/user/thumbnail/" + userStore.user.img) : avatarPlaceHolder,
    );
